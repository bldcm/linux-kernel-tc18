...
static int emac_adjust_to_link(struct ocp_enet_private *fep)
{
	...
	printk(KERN_INFO "%s: Speed: %d, %s duplex (auto-neg=%i).\n",
	       fep->ndev->name, speed, full_duplex ? "Full" : "Half", fep->want_autoneg);
	if (fep->opened)
...
#ifdef CONFIG_TC18
static int emac_adjust_phyleds(struct ocp_enet_private *fep, int link, int linkchanged)
{
	/* assume the link mode is already set correctly by emac_adjust_to_link */
	int speed = fep->phy_mii.speed;

	/* Switch to 100Mbit functioning if link is down */
	if (!link)
		speed = SPEED_100;

	switch (speed) {
	case SPEED_10:
		/* Left LED: blink; Right LED: Activity (normal mode)*/
		if (fep->status_led_active) {
			phy_write(&fep->phy_mii , MII_DP83848C_LEDCR, LEDCR_LNK_1);
			fep->status_led_active = 0;
		} else {
			phy_write(&fep->phy_mii , MII_DP83848C_LEDCR, LEDCR_LNK_0);
			fep->status_led_active = 1;
		}
		break;
	case SPEED_100:
	default:
		/* Set both LEDs to Normal function */
		if (linkchanged) { 
			phy_write(&fep->phy_mii , MII_DP83848C_LEDCR, LEDCR_NORMAL);
			fep->status_led_active = 1;
		}
	}
	return 0;
...
static void eth_send_gratuitous_arp(struct net_device *dev)
{
	 ...
	 */
#ifdef CONFIG_INET
	//LINK_DEBUG( "Sending gratuitous ARP \n");
	rcu_read_lock();
...
#ifdef CONFIG_TC18
	emac_adjust_phyleds(fep,link,linkchanged);
#endif
	if (link&&(fep->send_grat_arp)) {
		switch (fep->send_grat_arp) {
		case 2:	// send first grat arp
			printk(KERN_INFO "%s: 1. Sending gratuitous ARP \n", fep->ndev->name);
			eth_send_gratuitous_arp(fep->ndev);
			break;
		case 10:	// send second grat arp
			printk(KERN_INFO "%s: 2. Sending gratuitous ARP \n", fep->ndev->name);
			eth_send_gratuitous_arp(fep->ndev);
			fep->send_grat_arp=0;
			break;
		}
		if (fep->send_grat_arp)
			fep->send_grat_arp++;
	}
}
...
static int emac_init_device(struct ocp_device *ocpdev, struct ibm_ocp_mal *mal)
{
	...
	if (ocpdev->def->index == 0) {
		/* Set initial Mode for first LAN from command_line arguments. Fix of SPR91160 
		   (Mode for LAN2 will be set in script bb6/etc/init.d/rcC) */
		
		const char *sz_lan1_mode = strstr(saved_command_line, " tc_lan1_mode=");
		if (sz_lan1_mode != NULL) {
			sz_lan1_mode += 14;     // add strlen(" tc_lan1_mode=")
			printk("eth%d: Set LAN mode to \'%c\'\n", ocpdev->def->index, sz_lan1_mode[0]);
			switch (sz_lan1_mode[0]) {
				case '2': ep->want_autoneg = 0; ep->phy_mii.duplex = DUPLEX_HALF; ep->phy_mii.speed =  SPEED_10; break;
				case '3': ep->want_autoneg = 0; ep->phy_mii.duplex = DUPLEX_FULL; ep->phy_mii.speed =  SPEED_10; break;
				case '4': ep->want_autoneg = 0; ep->phy_mii.duplex = DUPLEX_HALF; ep->phy_mii.speed = SPEED_100; break;
				case '5': ep->want_autoneg = 0; ep->phy_mii.duplex = DUPLEX_FULL; ep->phy_mii.speed = SPEED_100; break;
			}
		}
	}
