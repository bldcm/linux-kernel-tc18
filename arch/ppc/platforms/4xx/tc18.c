...
#elif defined(CONFIG_TC18)
static const int bamboo = 0;
static const char cpu_name[] = {"440EP"};
static const char brd_name[] = {"TC18"};
#else
static const int bamboo = 0;
static const char cpu_name[] = {"440EP"};
...
static inline int
tc18_map_irq(struct pci_dev *dev, unsigned char idsel, unsigned char pin)
{
	 ...
	 */
	{
		{ 26, 26, 26, 26 },	/* IDSEL 1 - PCI Slot 0 DSP */
		{ 26, 26, 26, 26 },	/* IDSEL 2 - PCI Slot 1 USB */
		{ 26, 26, 26, 26 },	/* IDSEL 3 - PCI Slot 2 none */
...
yosemite_setup_hose(void)
{
	...
	ppc_md.pci_map_irq = tc18_map_irq;
}
...
static struct mtd_partition tc18_partitions[] = {
	{
	 .name = "ubootenv",
	 .size = 0x00020000,
	 .offset = 0x0,
...
static int __init tc18_setup_mtd(void)
{

	physmap_set_partitions(tc18_partitions, 3);
	physmap_set_partitions_flash2(tc18_partitions, 3);
	return 0;
}
...
static int __init tc18_setup_mtd(void)
{
	...
	physmap_set_partitions(tc18_partitions, 1);
	return 0;
}
#endif 


arch_initcall(tc18_setup_mtd);
#endif
...
yosemite_setup_arch(void)
{
	 ...
	 * --ebs
	 */
	ibm440gx_get_clocks(&clocks, TC18_SYSCLK, 6 * 1843200);
	...
	/* Identify the system */
	printk("PowerPC %s %s Platform\n", cur_cpu_name(), brd_name);
}
